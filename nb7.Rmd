...
library(ggbeeswarm)
source("./code/pb2gen.txt")
source("./code/onesampb.txt")
source("./code/wmwpb.txt")
source("./code/summarySE.txt") # Define summary function
# Code from: http://www.cookbook-r.com/Graphs/Plotting_means_and_error_bars_(ggplot2)/
```
...
pb2gen(husbands, wives, alpha=.05, nboot=2000, est=median, SEED=FALSE)
```
...
onesampb(husbands - wives, alpha=.05, nboot=2000, est=median, SEED=FALSE)
```
...
```{r}
onesampb(cdiff, alpha=.05, nboot=2000, est=pxlz, SEED=FALSE)
```
...
```{r}
out <- wmwpb(control,ozone,est=median,alpha=.05,nboot=2000,SEED=FALSE)
```
...
`cid()` also returns the complementary probability P(X<Y) and the robust ordinal measure of effect size known as Cliff's delta = P(X>Y) - P(X<Y).
