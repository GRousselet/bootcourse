...
source("./code/trimcibt.txt")
source("./code/yuenbt.txt")
source("./code/yuen.txt")
source("./code/sintv2.txt") # one-sample inference on the median
...
for(iter in 1:nsim){
  ...
  boot.q975[iter] <- quantile(boot.samp.dist[iter,], probs = 0.975)
}
...
print(c(npm/S,nptm/S,npmd/S))
```
The results show power of `r round(100*npm/i, digits=1)`% for the mean, `r round(100*nptm/i, digits=1)` for 20% trimmed mean, and `r round(100*npmd/i, digits=1)` for the median.
